...
hours <- seq(from=as.POSIXct("1979-10-01 01:00"), to=as.POSIXct("2019-09-01 00:00"), 
...
ggplot(df_dly, aes(dowy, wy, height = cfs, group=as.factor(wy), fill = anntaf))+
  ...
  scale_fill_viridis(name = "taf/yr") + theme_gray() + scale_x_continuous(breaks = breaks, labels = labels,
                                        sec.axis = dup_axis(name = NULL), expand = c(0,0)) +
  labs(x = NULL) + labs(y="water year") +  theme(legend.key.width = unit(0.5, "in"), legend.key.height = unit(1, "in")) +
  scale_y_continuous(sec.axis = dup_axis(name = NULL))
#scale_fill_distiller(palette = "Spectral", name = "taf/yr", direction = -1 ) 
ggsave("chvc1_hetc1_lnrc1_dlymeansqin_anntaf.png", width = 21, height = 11, units = "in", dpi = 300)
